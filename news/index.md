# Changelog

## indiedown 0.1.1.9049

### Bug fixes

#### ci

- Emit empty package matrix when there are no (rev)deps.

### fledge

- Bump version to 0.1.1.9043
  ([\#93](https://github.com/cynkra/indiedown/issues/93)).

## indiedown 0.1.1.9048

### Continuous integration

- Run on Ubuntu 26.04.

- Align workflows with template.

## indiedown 0.1.1.9047

### Chore

- Apply air formatting, add `air.toml`
  ([\#101](https://github.com/cynkra/indiedown/issues/101)).

## indiedown 0.1.1.9046

### Chore

- Auto-update from GitHub Actions
  ([\#99](https://github.com/cynkra/indiedown/issues/99)).

## indiedown 0.1.1.9045

### Continuous integration

- Update ccache-action reference.

- Bump action version.

## indiedown 0.1.1.9044

### fledge

- Bump version to 0.1.1.9042
  ([\#86](https://github.com/cynkra/indiedown/issues/86)).

## indiedown 0.1.1.9043

### Chore

- Add ccache to `.gitignore` and `.Rbuildignore`.

### Continuous integration

- Create snapshot update PR against correct branch.

- Add reference to `/apply-patch` workflow in commit message.

- Clarify rationale for not deploying on schedule.

- Only run fledge on pushes to main.

- Tweak fledge workflow and ccache action.

## indiedown 0.1.1.9042

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/25267027010>

### Continuous integration

- Cosmetics.

- Bump action versions.

- Install clang-format-21.

- Align fledge workflow.

- Harmonize.

### Documentation

- Add cynkra’s ROR ID
  ([\#65](https://github.com/cynkra/indiedown/issues/65)).

## indiedown 0.1.1.9041

### Continuous integration

- Fix comment ([\#84](https://github.com/cynkra/indiedown/issues/84)).

- Tweaks ([\#83](https://github.com/cynkra/indiedown/issues/83)).

- Test all R versions on branches that start with cran-
  ([\#82](https://github.com/cynkra/indiedown/issues/82)).

## indiedown 0.1.1.9040

### Continuous integration

- Install binaries from r-universe for dev workflow
  ([\#81](https://github.com/cynkra/indiedown/issues/81)).

## indiedown 0.1.1.9039

### Continuous integration

- Fix reviewdog and add commenting workflow
  ([\#80](https://github.com/cynkra/indiedown/issues/80)).

## indiedown 0.1.1.9038

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/17450815604>

### Continuous integration

- Use workflows for fledge
  ([\#79](https://github.com/cynkra/indiedown/issues/79)).

- Sync ([\#78](https://github.com/cynkra/indiedown/issues/78)).

- Use reviewdog for external PRs
  ([\#77](https://github.com/cynkra/indiedown/issues/77)).

- Cleanup and fix macOS
  ([\#76](https://github.com/cynkra/indiedown/issues/76)).

- Format with air, check detritus, better handling of `extra-packages`
  ([\#75](https://github.com/cynkra/indiedown/issues/75)).

## indiedown 0.1.1.9037

### Continuous integration

- Enhance permissions for workflow
  ([\#74](https://github.com/cynkra/indiedown/issues/74)).

## indiedown 0.1.1.9036

### Continuous integration

- Permissions, better tests for missing suggests, lints
  ([\#73](https://github.com/cynkra/indiedown/issues/73)).

## indiedown 0.1.1.9035

### Continuous integration

- Only fail covr builds if token is given
  ([\#72](https://github.com/cynkra/indiedown/issues/72)).

- Always use `_R_CHECK_FORCE_SUGGESTS_=false`
  ([\#71](https://github.com/cynkra/indiedown/issues/71)).

## indiedown 0.1.1.9034

### Continuous integration

- Correct installation of xml2
  ([\#70](https://github.com/cynkra/indiedown/issues/70)).

## indiedown 0.1.1.9033

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/14636209573>

### Continuous integration

- Explain ([\#69](https://github.com/cynkra/indiedown/issues/69)).

- Add xml2 for covr, print testthat results
  ([\#68](https://github.com/cynkra/indiedown/issues/68)).

- Fix ([\#67](https://github.com/cynkra/indiedown/issues/67)).

- Sync ([\#66](https://github.com/cynkra/indiedown/issues/66)).

## indiedown 0.1.1.9032

### Continuous integration

- Avoid failure in fledge workflow if no changes
  ([\#62](https://github.com/cynkra/indiedown/issues/62)).

## indiedown 0.1.1.9031

### Continuous integration

- Fetch tags for fledge workflow to avoid unnecessary NEWS entries
  ([\#61](https://github.com/cynkra/indiedown/issues/61)).

## indiedown 0.1.1.9030

### Continuous integration

- Use larger retry count for lock-threads workflow
  ([\#60](https://github.com/cynkra/indiedown/issues/60)).

## indiedown 0.1.1.9029

### Continuous integration

- Ignore errors when removing pkg-config on macOS
  ([\#59](https://github.com/cynkra/indiedown/issues/59)).

## indiedown 0.1.1.9028

### Continuous integration

- Explicit permissions
  ([\#58](https://github.com/cynkra/indiedown/issues/58)).

## indiedown 0.1.1.9027

### Continuous integration

- Use styler from main branch
  ([\#57](https://github.com/cynkra/indiedown/issues/57)).

## indiedown 0.1.1.9026

### Continuous integration

- Need to install R on Ubuntu 24.04
  ([\#56](https://github.com/cynkra/indiedown/issues/56)).

- Use Ubuntu 24.04 and styler PR
  ([\#54](https://github.com/cynkra/indiedown/issues/54)).

### Uncategorized

- PLACEHOLDER <https://github.com/cynkra/indiedown/pull/16>
  ([\#16](https://github.com/cynkra/indiedown/issues/16)).

## indiedown 0.1.1.9025

### Continuous integration

- Correctly detect branch protection
  ([\#53](https://github.com/cynkra/indiedown/issues/53)).

## indiedown 0.1.1.9024

### Continuous integration

- Use stable pak
  ([\#52](https://github.com/cynkra/indiedown/issues/52)).

## indiedown 0.1.1.9023

### Continuous integration

- Trigger run ([\#51](https://github.com/cynkra/indiedown/issues/51)).

  - ci: Trigger run

  - ci: Latest changes

## indiedown 0.1.1.9022

### Continuous integration

- Use pkgdown branch
  ([\#50](https://github.com/cynkra/indiedown/issues/50)).

  - ci: Use pkgdown branch

  - ci: Updates from duckdb

  - ci: Trigger run

## indiedown 0.1.1.9021

### Continuous integration

- Install via R CMD INSTALL ., not pak
  ([\#49](https://github.com/cynkra/indiedown/issues/49)).

  - ci: Install via R CMD INSTALL ., not pak

  - ci: Bump version of upload-artifact action

## indiedown 0.1.1.9020

### Continuous integration

- Install local package for pkgdown builds.

- Improve support for protected branches with fledge.

- Improve support for protected branches, without fledge.

## indiedown 0.1.1.9019

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/10425483323>

### Continuous integration

- Sync with latest developments.

## indiedown 0.1.1.9018

### Continuous integration

- Use v2 instead of master.

## indiedown 0.1.1.9017

### Continuous integration

- Inline action.

## indiedown 0.1.1.9016

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/10207825578>

### Continuous integration

- Use dev roxygen2 and decor.

## indiedown 0.1.1.9015

### Continuous integration

- Fix on Windows, tweak lock workflow.

## indiedown 0.1.1.9014

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/9727973619>

## indiedown 0.1.1.9013

### Chore

- Auto-update from GitHub Actions.

  Run: <https://github.com/cynkra/indiedown/actions/runs/9692290881>

### Continuous integration

- Avoid checking bashisms on Windows.

- Better commit message.

- Bump versions, better default, consume custom matrix.

- Recent updates.

## indiedown 0.1.1.9012

### Documentation

- Fix url ([\#28](https://github.com/cynkra/indiedown/issues/28)).

## indiedown 0.1.1.9011

### Chore

#### deps

- Update peter-evans/create-pull-request action to v6.

## indiedown 0.1.1.9010

### Chore

#### deps

- Update nick-fields/retry action to v3.

## indiedown 0.1.1.9009

### Chore

#### deps

- Update dessant/lock-threads action to v5.

#### deps

- Update actions/cache action to v4.

### Documentation

- Set BS version explicitly for now.

  <https://github.com/cynkra/cynkratemplate/issues/53>

## indiedown 0.1.1.9008

- Internal changes only.

## indiedown 0.1.1.9007

- Internal changes only.

## indiedown 0.1.1.9006

- Merge pull request
  [\#39](https://github.com/cynkra/indiedown/issues/39) from
  cynkra/snapshot-main-rcc-smoke-null.

## indiedown 0.1.1.9005

### Chore

- Build-ignore `renovate.json`.

### Uncategorized

- Merge pull request
  [\#30](https://github.com/cynkra/indiedown/issues/30) from
  cynkra/renovate/configure.

  Configure Renovate

## indiedown 0.1.1.9004

- Internal changes only.

## indiedown 0.1.1.9003

- Internal changes only.

## indiedown 0.1.1.9002

### Chore

- Avoid tests on R \< 4.0 on GHA.

## indiedown 0.1.1.9001

- Internal changes only.

## indiedown 0.1.1.9000

### simplify

- Substitute \<\<\>\> in the template.

### Uncategorized

- Harmonize yaml formatting.

- Revert changes to matrix section.

- Merge pull request
  [\#27](https://github.com/cynkra/indiedown/issues/27) from
  cynkra/f-backto-cleveref.

Back to cleveref

- Merge pull request
  [\#26](https://github.com/cynkra/indiedown/issues/26) from
  cynkra/b-cleveref.

Comment out cleveref since not used actively

- Merge pull request
  [\#25](https://github.com/cynkra/indiedown/issues/25) from
  cynkra/tidyout.

removed tidyverse library call

- Merge pull request
  [\#17](https://github.com/cynkra/indiedown/issues/17) from
  cynkra/b-workflow-2.

- Merge pull request
  [\#15](https://github.com/cynkra/indiedown/issues/15) from
  cynkra/b-workflows.

- Reduce parallelism.

- Also check dev on cran-\* branches.

- Update hash key for dev.

- Remove R 3.3.

- Merge pull request
  [\#14](https://github.com/cynkra/indiedown/issues/14) from
  cynkra/dr_down.

Dr down

- Merge pull request [\#1](https://github.com/cynkra/indiedown/issues/1)
  from christophsax/more-pandoc.

More pandoc

## indiedown 0.1.1

CRAN release: 2021-03-22

- CRAN feedback: better DESCRIPTION, extended documentation, examples.

## indiedown 0.1.0

Initial release.

- [`create_indiedown_package()`](https://indiedown.cynkra.com/reference/create_indiedown_package.md)
  creates a ready-to-customize R Markdown template package.
- [`use_indiedown_gfonts()`](https://indiedown.cynkra.com/reference/use_indiedown_gfonts.md)
  adds Google fonts to a template package.
