...
The function `cd_page_title()` creates an example title page that can be adjusted to your needs.
Note that the example uses raw strings, which are only available in R >= 4.
They make it easy to write LaTeX code directly in R.
See details in `?indiedown_glue` for methods that work well with R < 4.
...
Keep all corporate design elements as R code.
This keeps the LaTeX code simple and allows flexible extensions.
