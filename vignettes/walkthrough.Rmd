---
title: "Walkthrough: Build your own RMarkdown Template"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Walkthrough: Build your own RMarkdown Template}
...
`vignette("intro")` has shown how to set up a package skeleton with indiedown. This article teaches you how to apply customiztions to produce a template that fits your needs.
...
A full list of options is given in the [LaTeX section of the Padoc manual](https://pandoc.org/MANUAL.html#variables-for-latex).
...
Note that `<<indiedown_path>>` will be automatically substituted with the actual location of `inst/indiedown`. That is why we can include *assets* that are picked up by the template. The default template inclues the free Roboto font, which is located at `inst/indiedown`.
...
The function reliess on the [gfonts](https://CRAN.R-project.org/package=gfonts) package to download and install the 'Noto Sans JP' font.
...
Let's turn to the second customization point, `preamble.tex`. This contains LaTeX code that is run at the begining. Add everything here that cannot be changed in the `default.yaml`. We want to use slightly bigger headers, and change the 'titlesec' line to:

```tex
\usepackage[medium]{titlesec}
```

As `default.yaml`, it supports substitution of `<<indiedown_path>>`, so you could link to images here as well. Most of the time, you do not want do too many thing here.
...
As a final step, we build a customized title page, by modifiying the function definition of `R/cd_page_title`. The function writes LaTeX code that will be added to the `.Rmd` when `cd_page_title()` is called.
...
The `indiedown_glue()` helper function uses the [glue](https://CRAN.R-project.org/package=glue) package to substitue variables in the string. That way, variables such as `title` or `subtitle` will be substituted in the LaTeX code.

With `indiedown_path_tex()`, you can also refer to the path of `inst/indiedown`, and use assets from the package. In the example, we use this to include a logo. Now, we don't want to use a logo but rather a photography of a beautiful parrot. We download the image from [unsplash.com](https://unsplash.com/photos/gGC63oug3iY) in medium quality and store it in `res/mikhail-vasilyev-gGC63oug3iY-unsplash.jpg`. Then, we change the function definition as follows:
...
indiedown provides a framework to easily generate customized RMarkdow templates. Templates can be customized by the full power of Pandoc of LaTeX, through various points of customizations.

This walk-through introduced two of them.
For more details and a description of the third one, see `vignettes("customize")`.
